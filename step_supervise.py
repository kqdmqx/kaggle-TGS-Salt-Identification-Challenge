...
for i, (train_idx, valid_idx) in enumerate(kfold_index):
    ...
    early_stopping = EarlyStopping(monitor=metric_name, mode='max', patience=12, verbose=1)
    ...
    # c = SGD(lr=0.0002, momentum=.9, decay=1e-3)
    # model = load_model_bcelovasz2lovasz_deep(pre_model_filepath, c)  # start_neurons = 32
    model = load_model_with_allco(pre_model_filepath)
    model = change_deep_opt3(model, c)
    # model = change_deep_opt2(model, c)
    print("load model.")

    def train():
        return model.fit_generator(generate_batch_data_random_ref128depth_deep(X_train, Y_train, train_batch_size),            # try use aug
